...
function submitGenre(event) {
  ...
  const selectValue: string = selectGenres.value;
...
function findMovieByGenre(genre) {
  axios
    ...
    .then((response) => {
      ...
      const genreID: number = displayGenresID(response);
      const moviesURL: string = `https://api.themoviedb.org/3/discover/movie?api_key=2d0391dda9aa7c639004867f4b4bb58d&with_genres=${genreID}`;
    ...
    .then((response) => {
      ...
      const selectLangValue: string = selectLanguage.value;
